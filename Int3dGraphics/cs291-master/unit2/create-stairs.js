...
function createStairs() {
	...
	var vlasty=stepMesh.position.y
	var vlastz=stepMesh.position.z
	...
	var hlasty=stepMesh.position.y
	var hlastz=stepMesh.position.z
	...
	for (i=0; i<5; i++)
	{
		stepMesh = new THREE.Mesh( stepVertical, stepMaterialVertical );
		stepMesh.position.x = 0;			// centered at origin
		stepMesh.position.y = vlasty+verticalStepHeight+stepThickness;	// half of height: put it above ground plane
		stepMesh.position.z = vlastz+ stepWidth-2*stepThickness-stepThickness;			// centered at origin
		scene.add( stepMesh );
		vlasty=stepMesh.position.y
		vlastz=stepMesh.position.z
	}
	

	// Make and position the horizontal steps
	for (i=0; i<5; i++)
	{
	stepMesh = new THREE.Mesh( stepHorizontal, stepMaterialHorizontal );
	stepMesh.position.x = 0;
	// Push up by half of horizontal step's height, plus vertical step's height
	stepMesh.position.y = hlasty+verticalStepHeight + stepThickness;;
	// Push step forward by half the depth, minus half the vertical step's thickness
	stepMesh.position.z = hlastz+stepWidth - 3*stepThickness;
	scene.add( stepMesh );
	hlasty=stepMesh.position.y
	hlastz=stepMesh.position.z
	}
}
