...
#twiddle
tol = 0.008
p= [weight_data-(weight_data/6),weight_smooth-(weight_smooth/6),p_gain-(p_gain/6),d_gain-(d_gain/6)]
dp=[(weight_data/7),weight_smooth/7,p_gain/7,d_gain/7]
best_err= main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
       p[0], p[1], p[2], p[3])
n=0
while sum(dp) >tol:
    for i in range(len(p)):
        p[i]+=dp[i]
        err =main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
       p[0], p[1], p[2], p[3])
       ...
       p[0], p[1], p[2], p[3])
            ...
            if err[2]< best_err[2]:
                ...
                dp[i]*=0.9
    n+=1
    print 'Twiddle #',n, p,'-->',best_err[2]
            
#twiddle return p
print "final values:", p,dp
